...
GAMMA_PRM = 500.0
D = 2
COLLISION_LOOKAHEAD_FRAMES = 3
ROBOT_SEPARATION_BUFFER = 8
COLLISION_REPLAN_COOLDOWN_FRAMES = 12
...
def check_robot_collisions_with_lookahead(robots, lookahead_frames=1):
    """
    Predict robot-robot collisions by extrapolating each robot toward its
    current target waypoint. Returns robot index pairs that are colliding now
    or are predicted to collide within the lookahead window.
    """
    collisions = set()
    dt = 1.0 / 60.0
        ...
        for robot in robots:
            ...
            if target:
                ...
                if distance_to_target > 0:
                    distance_to_move = min(robot.speed * time_offset, distance_to_target)
                ...
                future_positions.append(robot.position)

        for i in range(len(robots)):
            for j in range(i + 1, len(robots)):
                min_distance = robots[i].radius + robots[j].radius
                if dist(future_positions[i], future_positions[j]) < min_distance:
                    collisions.add((i, j))
...
def is_robot_position_free(position, robot, obstacles):
    x, y = position
    if (
        x < robot.radius
        or x > WINDOW_WIDTH - robot.radius
        or y < robot.radius
        or y > WINDOW_HEIGHT - robot.radius
    ):
        return False

    for obs in obstacles:
        inflated = obs.get_inflated_vertices()
        if check_collision(position, robot.radius, inflated):
            return False

    return True

def separate_colliding_robots(env, collisions):
    """
    Move colliding robots a small distance away from each other before rebuilding
    PRMs. Without this, replanning can repeatedly detect the same unresolved
    overlap before robots have time to follow their new paths.
    """
    for _ in range(SEPARATION_ITERATIONS):
        moved_any = False

        for i, j in collisions:
            robot_a = env.robots[i]
            robot_b = env.robots[j]

            dx = robot_a.position[0] - robot_b.position[0]
            dy = robot_a.position[1] - robot_b.position[1]
            ...
            required_distance = robot_a.radius + robot_b.radius + ROBOT_SEPARATION_BUFFER
            if distance >= required_distance:
                continue

            nx = dx / distance
            ny = dy / distance
            push_distance = (required_distance - distance) / 2.0

            candidate_a = (
                clamp(robot_a.position[0] + nx * push_distance, robot_a.radius, WINDOW_WIDTH - robot_a.radius),
                clamp(robot_a.position[1] + ny * push_distance, robot_a.radius, WINDOW_HEIGHT - robot_a.radius),
            )
            candidate_b = (
                clamp(robot_b.position[0] - nx * push_distance, robot_b.radius, WINDOW_WIDTH - robot_b.radius),
                clamp(robot_b.position[1] - ny * push_distance, robot_b.radius, WINDOW_HEIGHT - robot_b.radius),
            )

            if is_robot_position_free(candidate_a, robot_a, env.obstacles):
                robot_a.position = candidate_a
                moved_any = True

            if is_robot_position_free(candidate_b, robot_b, env.obstacles):
...
def plan_paths_for_all_robots(env):
    """Synchronously rebuild one PRM per robot from its current position."""
    for i, robot in enumerate(env.robots):
        prm = PRM(env.obstacles, robot.position, env.goal[0], N_SAMPLES, GAMMA_PRM, D)
        path = prm.dijkstra()
        if path:
            robot.set_path(path)
            print(f"Path found for robot {i} at {robot.position}: length {len(path)}")
        else:
            robot.path = None
            robot.reached_goal = True
            print(f"No path found for robot {i} at {robot.position}")
...
def main():
    ...
    while running:
        ...
        if collision_check_cooldown > 0:
            collision_check_cooldown -= 1
            collisions = set()
        else:
            collisions = check_robot_collisions_with_lookahead(env.robots, COLLISION_LOOKAHEAD_FRAMES)

        if collisions:
            print(f"Robot collision detected between pairs {sorted(collisions)}. Replanning all robots...")
            separate_colliding_robots(env, collisions)
            plan_paths_for_all_robots(env)
            collision_check_cooldown = COLLISION_REPLAN_COOLDOWN_FRAMES
            env.render(screen)
            pygame.display.flip()
            continue
